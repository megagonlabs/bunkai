...
class TestTsukubaSplitter(unittest.TestCase):
    ...
    def test_okay(self):
        ...
        self.assertTrue(isinstance(splitter_obj.eos(test_sentence_1_ok), Annotations))
        self.assertEqual(len(splitter_obj.eos(test_sentence_1_ok).get_final_layer()), 7)
        self.assertEqual(len(splitter_obj.find_eos(test_sentence_1_ok)), 7)
        self.assertEqual(len(list(splitter_obj(test_sentence_1_ok))), 7)
