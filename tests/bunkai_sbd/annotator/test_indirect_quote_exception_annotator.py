...
class TestMorphAnnotator(unittest.TestCase):
    ...
    def init_tokenized_layer(self, text: str) -> Annotations:
        ...
        pipeline = [BasicRule(), FaceMarkDetector(), EmotionExpressionAnnotator(), EmojiAnnotator()]
    ...
    def test_if_linebreak_annotator_false(self):
        ...
        ] + list(default_annotation.get_annotation_layer("first"))
        default_annotation.add_annotation_layer(
            annotations=pseudo_out_linebreak, annotator_name=LinebreakAnnotator.__name__
        )
