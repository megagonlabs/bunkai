...
class TestCli(unittest.TestCase):
    def setUp(self) -> None:
        self.seq_test_case = [
            ...
            NewlineTestCase('11時前に着きましたお部屋の準備ができているとの事で早くにチェックインでき足腰が悪いので大変あり難かったです、'
                            '部屋も広く掃除も行き届いていました、'
                            'お風呂も色々あり特に薬草風呂が良かった露天風呂はあまり開放感はありません街中なのでしかたがないです、', 1, []),
    ...
    def test_cli(self):
        ...
        for test_case in self.seq_test_case:
            output = ''.join([o for o in
                              cli_module.run(model, test_case.text.replace('\n', bunkai.constant.METACHAR_LINE_BREAK))])
