...
from bunkai.algorithm.bunkai_sbd.annotator import (
    BasicRule, DotExceptionAnnotator, EmojiAnnotator,
    EmotionExpressionAnnotator, FaceMarkDetector,
    IndirectQuoteExceptionAnnotator, LinebreakAnnotator,
    LinebreakForceAnnotator, MorphAnnotatorJanome, NumberExceptionAnnotator)
...
class BunkaiPipeline(AnnotatorPipeline):
    def check(self) -> bool:
        ...
        if class_name2order[MorphAnnotatorJanome.__name__] > class_name2order[IndirectQuoteExceptionAnnotator.__name__]:
            raise RuleOrderException(f'{MorphAnnotatorJanome.__name__}  must be'
                                     f' before {IndirectQuoteExceptionAnnotator.__name__}')
        if LinebreakAnnotator.__name__ in class_name2order:
            if class_name2order[MorphAnnotatorJanome.__name__] > class_name2order[LinebreakAnnotator.__name__]:
                raise RuleOrderException(f'{MorphAnnotatorJanome.__name__}  must be'
                                         f' before {LinebreakAnnotator.__name__}')
            if class_name2order[LinebreakAnnotator.__name__] > class_name2order[IndirectQuoteExceptionAnnotator.__name__]:
                raise RuleOrderException(f'{LinebreakAnnotator.__name__}  must be'
                                         f' before {IndirectQuoteExceptionAnnotator.__name__}')
        return True
